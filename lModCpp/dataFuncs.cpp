...
void pFitFunc(int particles, pMMHres results, obsDatX obsDat, modParms prms, string outputFile, string dFunc) {
	...
	vector<double> rainfall_06 = txtReader("\\\\qdrive.dide.ic.ac.uk\\homes\\ALM210\\Imperial\\lModCpp\\Data\\rf06.txt", prms.dt);
	...
	prms.z9 = medianFnc(results.z9);
	prms.z10 = medianFnc(results.z10);
	...
	prms.sf9 = medianFnc(results.sf9);
	prms.sf10 = medianFnc(results.sf10);
...
double resultsWriter(string fileName, string outputFolder, pMMHres results) {
	...
	for (auto iter = 0; iter != size(results.ll); ++iter) {
		myfile << results.uoE.at(iter) << " " << results.uoL.at(iter) << " " << results.uP.at(iter) << " " << results.Y.at(iter) << " " << results.w.at(iter)
			<< " " << results.n.at(iter) << " " << results.z1.at(iter) << " " << results.z2.at(iter) << " " << results.z3.at(iter) << " " << results.z4.at(iter) << " " << results.z5.at(iter) << " " << results.z6.at(iter) << " " << results.z7.at(iter) << " " << results.z8.at(iter) << " " << results.z9.at(iter) << " " << results.z10.at(iter)
			<< " " << results.sf1.at(iter) << " " << results.sf2.at(iter) << " " << results.sf3.at(iter) << " " << results.sf4.at(iter) << " " << results.sf5.at(iter) << " " << results.sf6.at(iter) << " " << results.sf7.at(iter) << " " << results.sf8.at(iter) << " " << results.sf9.at(iter) << " " << results.sf10.at(iter) <<
